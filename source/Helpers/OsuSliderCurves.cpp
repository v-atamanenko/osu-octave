...
#define osu_slider_curve_points_separation 2.5f // slider body curve approximation step width in osu!pixels, don't set this lower than around 1.5
#define osu_slider_curve_max_points 1000.0f // maximum number of allowed interpolated curve points. quality will be forced to go down if a slider has more steps than this
#define osu_slider_curve_max_length 65536/2 // maximum slider length in osu!pixels (i.e. pixelLength). also used to clamp all (control-)point coordinates to sane values.
...
{
    return createCurve(osuSliderCurveType, controlPoints, pixelLength, osu_slider_curve_points_separation);
}
...
OsuSliderCurveTypeCentripetalCatmullRom::OsuSliderCurveTypeCentripetalCatmullRom(const std::vector<Vector2> &points) : OsuSliderCurveType()
{
    if (points.size() != 4)
    {
        fprintf(stderr, "OsuSliderCurveTypeCentripetalCatmullRom() Error: points.size() != 4!!!\n");
        return;
    }
...
{
    m_iNCurve = std::min((int)(m_fPixelLength / std::clamp<float>(curvePointsSeparation, 1.0f, 100.0f)), (int)osu_slider_curve_max_points);
}
...
void OsuSliderCurveEqualDistanceMulti::init(const std::vector<OsuSliderCurveType*> &curvesList)
{
    ...
    {
        fprintf(stderr, "OsuSliderCurveEqualDistanceMulti::init() Error: curvesList.size() == 0!!!\n");
        return;
    }
    ...
    OsuSliderCurveType *curCurve = curvesList[curCurveIndex];
    {
        ...
        {
            fprintf(stderr, "OsuSliderCurveEqualDistanceMulti::init() Error: curCurve->getCurvePoints().size() == 0!!!\n");
    ...
    for (int i=0; i<(m_iNCurve + 1); i++)
    {
        ...
        if (distanceAt - lastDistanceAt > 1)
        {
            ...
            m_curvePoints[i] = Vector2(std::lerp<float>(lastCurve.x, thisCurve.x, t), std::lerp<float>(lastCurve.y, thisCurve.y, t));
        }
        else
    ...
    if (m_curvePoints.size() == 0)
    {
        fprintf(stderr, "OsuSliderCurveEqualDistanceMulti::init() Error: m_curvePoints.size() == 0!!!\n");
...
Vector2 OsuSliderCurveEqualDistanceMulti::pointAt(float t)
{
    ...
    if (index >= m_iNCurve)
    {
        ...
        {
            fprintf(stderr, "OsuSliderCurveEqualDistanceMulti::pointAt() Error: Illegal index %i!!!\n", m_iNCurve);
            return Vector2(0, 0);
        }
    }
    else
...
Vector2 OsuSliderCurveEqualDistanceMulti::originalPointAt(float t)
{
    ...
    if (index >= m_iNCurve)
    {
        ...
        {
            fprintf(stderr, "OsuSliderCurveEqualDistanceMulti::originalPointAt() Error: Illegal index %i!!!\n", m_iNCurve);
            return Vector2(0, 0);
        }
    }
    else
...
OsuSliderCurveCircumscribedCircle::OsuSliderCurveCircumscribedCircle(std::vector<Vector2> controlPoints, float pixelLength, float curvePointsSeparation) : OsuSliderCurve(controlPoints, pixelLength)
{
    ...
    {
        fprintf(stderr, "OsuSliderCurveCircumscribedCircle() Error: controlPoints.size() != 3\n");
        return;
    }
    ...
    if (!isIn(m_fCalculationStartAngle, midAng, m_fCalculationEndAngle))
    {
        ...
        {
            fprintf(stderr, "OsuSliderCurveCircumscribedCircle() Error: Cannot find angles between midAng (%.3f %.3f %.3f)\n", m_fCalculationStartAngle, midAng, m_fCalculationEndAngle);
            return;
        }
    }
    ...
    // calculate points
    const float steps = std::min(m_fPixelLength / (std::clamp<float>(curvePointsSeparation, 1.0f, 100.0f)), (float)osu_slider_curve_max_points);
...
{
    const float sanityRange = (float)osu_slider_curve_max_length; // NOTE: added to fix some aspire problems (endless drawFollowPoints and star calc etc.)
...
Vector2 OsuSliderCurveCircumscribedCircle::intersect(Vector2 a, Vector2 ta, Vector2 b, Vector2 tb)
{
    ...
    {
        fprintf(stderr, "OsuSliderCurveCircumscribedCircle::intersect() Error: Vectors are parallel!!!\n");
        return Vector2(0, 0);
    }
