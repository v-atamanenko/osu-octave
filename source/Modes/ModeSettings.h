...
public:
    ...
    StringSelector(int32_t x, int32_t y) {
        mX = x;
        mY = y;
        selectedString = new pText("default", FONT_CONSOLE, mX+13, mY+8, Skins::get_options().FontColor_SettingsLabel);
...
public:
    ...
    RadioButton(int32_t x, int32_t y, const std::string& false_value_label, const std::string& true_value_label, bool value) {
        ...
        falseValueLabel = new pText(false_value_label, FONT_PIXEL, mX+70, mY+16, Skins::get_options().FontColor_SettingsLabel);
        falseValueLabel->Z = -0.6f;
        falseValueLabel->Origin = ORIGIN_CENTER;
        trueValueLabel = new pText(true_value_label, FONT_PIXEL, mX+200, mY+16, Skins::get_options().FontColor_SettingsLabel);
